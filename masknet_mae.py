...
import einops
from vit_local import PatchEmbed, Block, TABlock
...
class CosineDiversityLoss(nn.Module):
    def __init__(self, scale_factor=0.1):
        super(CosineDiversityLoss, self).__init__()
    ...
    def forward(self, outputs):
        ...
        # Normalize outputs to unit vectors
        outputs = torch.nn.functional.normalize(outputs, p=2, dim=1)

        # Compute pairwise cosine similarity
        cosine_sim = torch.matmul(outputs, outputs.t())
        ...
        # Normalize loss by number of pairs and scale it
        num_pairs = n * (n - 1) / 2
        loss /= num_pairs
...
class FixedRatioLoss(nn.Module):
    def __init__(self, target_ratio=0.25):
        super(FixedRatioLoss, self).__init__()
        self.target_ratio = target_ratio

    def forward(self, outputs):
        # Ensure a fixed mask ratio
        mean_masking_output = outputs.mean(dim=-1)
        ratio_loss = torch.abs(mean_masking_output - self.target_ratio).mean()
...
class MaskingNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat((cls_tokens, x), dim=1)[:, 1:, :]
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
                 ...
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False, lambda_ratio =5.0):
        super().__init__()
        ...
        self.diversity_loss = CosineDiversityLoss()
        self.lambda_diversity = lambda_ratio
        self.lambda_FixedRatio = lambda_ratio
        self.fixedRatio_loss = FixedRatioLoss()
        # --------------------------------------------------------------------------
    ...
    def curriculum_masking(self, x, mask_ratio, train_mask):
        ...
        if train_mask:
            ids_shuffle = torch.argsort(mask_embedding, dim=1,
                                        descending=True)  # descend: small is remove, large is keep
            ids_restore = torch.argsort(ids_shuffle, dim=1)
    ...
    def forward_loss_mask(self, imgs, pred, masking_output, mask_ratio, ids_restore):
        ...
        fixedRatio_loss = self.lambda_FixedRatio * self.fixedRatio_loss(masking_output)

        return loss_reconst, fixedRatio_loss, loss_diversity
    ...
    def forward(self, imgs, mask_ratio=0.75, train_mask=False):
        ...
        if train_mask:
            loss_recon, fixedRatio_loss, loss_diversity = self.forward_loss_mask(imgs, pred, mask, mask_ratio,
                                                                                     ids_restore)
            return loss_recon, fixedRatio_loss, loss_diversity
        else:
...
# height_visual = 224
# width_visual = 224
# img1_input = torch.randn((batch_size, num_channels_visual, height_visual, width_visual))
# x = mae_vit_base_patch8()(img1_input, train_mask=True)
# print(x)